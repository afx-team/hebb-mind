"""hebb init — initialize a project directory."""
...
@click.option(
    ...
    "target_dir",
    default=None,
    help="Directory to initialize (default: HEBB_HOME or ~/.hebb/)",
)
@click.option("--force", is_flag=True, help="Overwrite existing config")
def init_cmd(target_dir: str | None, force: bool) -> None:
    """Initialize a hebb project directory.

    If --dir is not specified, defaults to ~/.hebb/ (global workspace).
    """
    target = Path(target_dir).resolve() if target_dir else default_init_target()
    config_path = target / "hebb.json"
    ...
    console.print()
    console.print("[bold]Hebb Mind initialized![/]")
    ...
    console.print("     [cyan]hebb setup[/]")
    console.print()
    console.print("  2. Or start directly with the current config:")
    console.print("     [cyan]hebb start[/]")
    console.print()
