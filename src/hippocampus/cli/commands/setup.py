"""hippocampus setup — prepare the default out-of-box environment."""
...
from hippocampus.cli.commands.init import default_init_target, initialize_workspace
...
def setup_cmd(ctx: click.Context, language: str, region: str, profile: str) -> None:
    """Prepare Hippocampus for first use without starting the service."""
    ...
    console.print()
    console.print("[bold]Hippocampus setup complete.[/]")
    console.print("Next steps:")
    console.print("  Start server:       [cyan]hippocampus start[/]")
    console.print("  Claude Code setup:  [cyan]hippocampus cc install --scope user[/]")
    console.print("  Codex setup:        [cyan]hippocampus codex install --scope user[/]")
    console.print("  Check health:       [cyan]hippocampus doctor[/]")
...
def _verify_model(model_id: str, hf_endpoint: str | None) -> int:
    ...
    asyncio.run(embedder.embed("hippocampus setup verification"))
