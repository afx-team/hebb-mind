...
3-tier priority:
  1. HIPPOCAMPUS_HOME env var  → explicit override
  2. ``home`` field in hippocampus.json or config file parent dir
  3. ~/.hippocampus/           → global default (OS user directory)

Data files (hippocampus.db, knowledge_graph.json, models/) always live
...
def get_default_home() -> Path:
    """Return OS-appropriate default home directory: ``~/.hippocampus/``."""
    return Path.home() / ".hippocampus"
...
def resolve_workspace(config_path: Path | None = None) -> Path:
    ...
    Priority:
        1. ``HIPPOCAMPUS_HOME`` environment variable (absolute path)
        2. ``home`` field in hippocampus.json (if set)
        3. Parent directory of *config_path* (or found ``hippocampus.json``)
        4. ``~/.hippocampus/`` (auto-created if missing)
