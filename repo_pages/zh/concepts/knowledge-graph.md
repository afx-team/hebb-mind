---
description: "Hebb Mind 在记忆巩固时自动构建基于标签的知识图谱（NetworkX），作为向量、关键词之外的第三路召回通道，并通过共现边扩展发现关联记忆。"
---

# 知识图谱

Hebb Mind 内置了基于标签的知识图谱，在记忆巩固过程中自动构建，用于增强检索和发现记忆之间的关联。

## 实现方式

- **图引擎**：NetworkX（内存中的图数据结构）
- **持久化**：JSON 文件（`knowledge_graph.json`），自动存储在工作目录中（文件路径由工作目录决定）
...
图谱路是三路检索通道之一（与向量路、关键词路并列），其结果会与另外两路一起经 RRF 融合、综合评分、cross-encoder 重排序。图谱路本身做的是基于标签的轻量召回，**而非语义检索**：

- 将搜索查询与图谱中的标签做匹配
- 沿共现边扩展到 1-hop 邻居标签
- 收集关联的 memory ID 交给融合阶段
