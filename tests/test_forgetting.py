...
class TestForgettingFormula:
    ...
    def test_high_access_extends_ttl(self):
        ...
        ttl_low = compute_ttl_hours(
            last_accessed_at=now, access_count=1, importance_score=5.0,
            base_ttl_hours=168, decay_factor=0.693, now=now,
        )
        ttl_high = compute_ttl_hours(
            last_accessed_at=now, access_count=100, importance_score=5.0,
            base_ttl_hours=168, decay_factor=0.693, now=now,
        )
    ...
    def test_old_access_reduces_ttl(self):
        ...
        ttl_old = compute_ttl_hours(
            last_accessed_at=now - timedelta(days=7), access_count=1, importance_score=5.0,
            base_ttl_hours=168, decay_factor=0.693, now=now,
        )
    ...
    def test_zero_importance(self):
        now = datetime.now(timezone.utc)
        ttl = compute_ttl_hours(
            last_accessed_at=now, access_count=1, importance_score=0.0,
            base_ttl_hours=168, decay_factor=0.693, now=now,
        )
        assert ttl == 0.0
    ...
    def test_never_negative(self):
        now = datetime.now(timezone.utc)
        ttl = compute_ttl_hours(
            last_accessed_at=now - timedelta(days=365), access_count=1,
            importance_score=1.0, base_ttl_hours=168, decay_factor=0.693, now=now,
        )
        assert ttl >= 0.0
