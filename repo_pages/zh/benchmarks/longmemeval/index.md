...
`single-session-preference` —— 抽象的推荐类问题（「能推荐些适合我现有摄影装备的配件吗？」）对上具体的用户陈述（「升级相机闪光灯」「想入一个三脚架」），两者几乎零 token 重叠 —— 曾经是本基准的地板。如今检索已到天花板：生产灌库镜像会为每条偏好短语额外写入一条合成记忆（与 `integrations/claude_code/write.py` 一致），补回了原始 embedding 相似度无法跨越的「查询↔语料」词汇鸿沟，剩下的排序由重排解决。
