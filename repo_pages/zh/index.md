...
features:
  - icon: ⚡
    title: 60 秒本地启动
    details: "pipx install + hebb setup + hebb service install。首次运行仅在尚未缓存时下载一个小 Embedding 模型（英文约 90MB / 多语言约 470MB）。SQLite + sentence-transformers，零外部服务。写入与混合检索完全不依赖 API Key。"
  - icon: 🧠
    title: 巩固时解决冲突
...
<div class="hippo-install">
  ...
  <div class="hippo-install-hint">首次运行会下载一个小模型（英文约 90MB / 多语言约 470MB），仅当尚未缓存时才下载 —— 英文 / <code>fast</code> 路径约一分钟，多语言略久。想要高质量的 bge 模型（1–2GB）？用 <code>hebb setup --profile best</code>。还没装 <code>pipx</code>？ <a href="/zh/guide/installation#如果还没装-pipx">一次性安装</a>（brew / apt / dnf / python -m pip）。然后打开 <a href="http://localhost:8321/">http://localhost:8321/</a>。需要 LLM 巩固时见 <a href="/zh/quick-start#路径-b-5-分钟-启用-llm-巩固">5 分钟路径</a>。</div>
</div>
