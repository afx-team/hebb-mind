"""hebb service — install/uninstall system service for auto-start."""
...
def _hebb_bin() -> str:
    """Return the absolute path to the hebb CLI binary."""
    # Prefer the executable that's currently running
    cli_path = shutil.which("hebb")
    if cli_path:
        return cli_path
    # Fallback: python -m invocation
    return f"{sys.executable} -m hebb.cli.main"
...
SYSTEMD_PATH = Path("/etc/systemd/system/hebb.service")
LAUNCHD_LABEL = "com.hebb.server"
...
def service_install() -> None:
    """Install Hebb Mind as a system service (auto-start on boot)."""
...
def _install_systemd() -> None:
    ...
    console.print(f"  Command: {_hebb_bin()} start")
    console.print()
    ...
    cmds = [
        ...
        ["systemctl", "enable", "hebb"],
        ["systemctl", "start", "hebb"],
    ]
    for cmd in cmds:
        try:
    ...
    console.print("  Status:  [cmd]systemctl status hebb[/]")
    console.print("  Logs:    [cmd]journalctl -u hebb -f[/]")
    console.print("  Stop:    [cmd]systemctl stop hebb[/]")
...
def _install_launchd() -> None:
    ...
    console.print("  Logs:    [cmd]tail -f /tmp/hebb.log[/]")
...
def _launchd_is_loaded() -> bool:
    """Return whether launchd knows about the Hebb Mind service."""
...
def _load_launchd_service() -> None:
    ...
    if not _launchd_is_loaded():
        console.print("[red]launchd did not register the Hebb Mind service.[/]")
        raise SystemExit(1)
