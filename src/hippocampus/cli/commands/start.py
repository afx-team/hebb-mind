...
def _start_daemon(host: str, port: int, reload: bool) -> None:
    ...
    # Build command: hippocampus start (without --daemon)
    cmd = [sys.executable, "-m", "hippocampus.cli.main", "start", "--host", host, "--port", str(port)]
    if reload:
        cmd.append("--reload")
    ...
    console.print(f"[bold green]Hippocampus started in daemon mode[/] (PID {process.pid})")
